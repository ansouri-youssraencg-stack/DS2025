...
Original file is located at
    https://colab.research.google.com/drive/1QZTqaVpUkBtrxpiV6qZaD0ms0zNHrfEm
"""
...
# The previous attempt to load from a URL failed with HTTP Error 404: Not Found.
# Instead, we will use the 'wine_quality' object already fetched using ucimlrepo.
# The 'wine_quality' object (from the previous cell) contains both red and white wine data.
# The 'wine_quality.data.original' DataFrame includes a 'color' column that specifies 'red' or 'white'.

# Create a DataFrame containing all features, target, and the 'color' column.
full_df = wine_quality.data.original

# Filter for white wine data, as the original problematic link was for 'winequality-white.csv'.
# We also drop the 'color' column from this filtered DataFrame, as it's typically not part of the features for analysis.
df = full_df[full_df['color'] == 'white'].drop(columns=['color'])
